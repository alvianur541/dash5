...
export function FieldNoteModal({
  ...
  // Pre-fill dari hasil ekstraksi AI saat modal dibuka (teknisi tinggal cek/edit)
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handlePolish = async () => {
    ...
    setPolishing(true);
    try {
      setNote(await polishFieldNote(note, model));
    } finally {
      setPolishing(false);
    }
  };
  ...
  const handleSubmit = async () => {
    if (note.trim().length < 10) { setError('Catatan terlalu pendek untuk disimpan.'); return; }
    ...
    if (res.ok && res.verdict === 'worthy') {
      ...
      setPhase('edit');
      setRejectReason(res.reason ?? 'Catatan belum cukup spesifik untuk masuk knowledge base. Tambah detail teknis lalu simpan lagi.');
    } else {
      setPhase('edit');
            ...
            <div className="flex items-center justify-between gap-3 px-4 pt-3 pb-2.5 border-b border-[var(--border-main)] shrink-0">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                <div className="flex flex-col min-w-0">
                  <p className="text-[13px] font-semibold text-[var(--text-primary)] truncate">Simpan ilmu lapangan</p>
                  <p className="text-[10.5px] text-[var(--text-muted)] mt-px truncate">{model} · ditangkap AI dari obrolanmu</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {phase === 'done' ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 px-6 py-10 text-center">
                ...
                <p className="text-[12px] text-[var(--text-muted)] leading-relaxed max-w-[290px]">
                  Ilmu kamu lolos kurasi AI dan langsung masuk knowledge base. Teknisi lain yang menanyakan hal serupa akan terbantu.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-3 flex flex-col gap-3">
                  <p className="text-[11.5px] text-[var(--text-muted)] leading-relaxed">
                    AI menangkap poin ini dari obrolanmu. Cek &amp; rapikan bila perlu, lalu simpan.
                  </p>
                  ...
                  <label className="flex flex-col gap-1">
                    <span className="flex items-center justify-between">
                      <span className="text-[11px] font-medium text-[var(--text-muted)]">Ilmu / trik lapangan</span>
                      <button
                    ...
                    <textarea
                      ...
                      rows={6}
                      placeholder="Ringkasan ilmu/trik lapangan…"
                ...
                <div className="flex items-center justify-end gap-2 px-3 py-2.5 border-t border-[var(--border-main)] shrink-0">
                  ...
                  >
                    ...
                    {phase === 'submitting' ? 'Menyimpan…' : 'Simpan'}
                  </button>
                </div>
              </>
            )}
